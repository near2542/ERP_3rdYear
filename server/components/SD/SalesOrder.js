...
router.get('/:id',async (req,res)=>
{
   ...
   where SDDoc.idDoc = ${req.params.id} and SDDoc.Type = 3 ; `,(err,result)=>
    {
...
{   
   ...
   const {idDoc,requestDate,description,idDiscount,idCustomer,customerPO} = req.body
    console.log(req.body);
    console.log('------------------------------this was in SDDoc Quotation------------------------------')
    DB.query(`INSERT INTO SDDoc values  
    ('','${todaydate}','${requestDate}','${description}','${idDiscount}',3,1,'${idDoc}',${customerPO},${idCustomer},null) ;`
            ,(err,result)=>
    {
        if(err) console.log(err)
        console.log(result);
