<template>
    <div class="middle">
       <nuxt-link to="/mm/invoices/"> <b-button variant="success">Go back to Invoices List</b-button> </nuxt-link>
       <div class="document">
            <header>
                <div><h1>Euphoria Speed</h1></div>
                <div><h3>Invoices<br>INV#{{requistion[0].idDoc}}</h3>
                    <p>reference from </p></div>
            </header>
           
            <div class="date">
            <p><strong>Pay on: </strong>{{requistion[0].request_date}}</p>
                </div>
                <!--------------- ------------->
            <div class="address">
...
<script>
  ...
  {
    const data = await $axios.$get(`/api/mm/invoices/${params.id}`)
    console.log(data)
